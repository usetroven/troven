...
| Auth | Supabase auth | Hosted auth UI, webhooks sync users to `Workspace` table |
...
**Rules:**
- `Workspace` is the top-level tenant. One Supabase user = one Workspace.
...
- In every protected tRPC procedure, derive `workspaceId` from `ctx.userId` via a `db.workspace.findUnique({ where: { supabaseId: ctx.userId } })` call — never trust a `workspaceId` passed from the client.
